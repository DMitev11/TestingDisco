...
#if WITH_STEAMCORE
	if (const FOnlineSubsystemSteamCore* SteamSub = static_cast<const FOnlineSubsystemSteamCore*>(IOnlineSubsystem::Get(STEAMCORE_SUBSYSTEM)))
	{
		const IOnlineSessionPtr SessionInt = SteamSub->GetSessionInterface();
