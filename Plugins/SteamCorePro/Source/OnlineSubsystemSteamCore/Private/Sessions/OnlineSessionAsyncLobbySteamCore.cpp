...
void FOnlineAsyncTaskSteamCoreFindLobbiesBase::CreateQuery()
{
	...
	m_SteamMatchmakingPtr->AddRequestLobbyListDistanceFilter(k_ELobbyDistanceFilterWorldwide);
...
void FOnlineAsyncEventSteamCoreLobbyInviteAccepted::Finalize()
{
	...
	if (SessionInt.IsValid() && !SessionInt->m_CurrentSessionSearch.IsValid())
	{
		...
		FOnlineAsyncTaskSteamCoreFindLobbiesForInviteSession* NewTask = new FOnlineAsyncTaskSteamCoreFindLobbiesForInviteSession(Subsystem, *m_LobbyId, SessionInt->m_CurrentSessionSearch, m_LocalUserNum, SessionInt->OnSessionUserInviteAcceptedDelegates);
